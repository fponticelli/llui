...
- `stateDiff` — JSON-Patch diff from pre-dispatch to post-drain state, populated for `dispatched` entries. Surfaces what actually changed without re-querying. The host renders it via `agentLog.connect().entryDiff(id)` (memoized accessor over the entry's diff) plus the `summarizeDiff` / `groupDiff` / `describeOp` helpers from `@llui/agent/client`.

The activity feed plus per-entry diff sidecar is what makes the agent's actions perceptible — the user sees not just _that_ something changed, but _what_ changed, in plain prose, without leaving the app.

### 5b.2 Visual attention layer (`agentAttention`)
