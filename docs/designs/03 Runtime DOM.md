...
**The accessor purity contract.** `items`, `key`, `branch.on`, `show.when`, `scope.on`, `child.props`, `foreign.props`, and binding accessors (`text(s => …)`, `el({attr: s => …})`) must all be **pure functions of their parameter**. The compiler computes a mask of state paths each accessor reads (only via `param.X` — see Architecture, "Compile-time field tracking") and the runtime gates structural reconcile and binding updates on that mask. A read of state outside the parameter — most commonly `sample(s2 => s2.X)` — is invisible to the analysis. The dep is silently lost: structural blocks gate out the change that should fire, binding accessors miss state updates, and `key` callbacks build keys that don't track outer state correctly. To depend on outer state inside an accessor, **lift it into the parameter**: bake the dep into `items`, return a wider object from `props`, etc. The runtime enforces this by throwing a targeted error when `sample()` is called from inside a known accessor; the `@llui/eslint-plugin-llui` rule `no-sample-in-accessor` catches the same antipattern at edit time.
