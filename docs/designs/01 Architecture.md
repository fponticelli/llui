...
export function toolbarView<S>(
  ...
  div({}, [
    ...
    show({ when: s => props.toolbar(s).menuOpen, render: () =>
...
**Headless component library: resolved — Zag.js via `@llui/zag` adapter.** Accessible UI components (Dialog, Select, Combobox, Menu, Tooltip, Tabs, etc.) are provided by Zag.js, a headless library built on Zag.js finite state machines. `@llui/zag` bridges Zag's FSM transitions into LLui's reactivity model and wraps individual components as LLui view functions. Zag machines are pure `(state, event) → state` functions — structurally aligned with TEA. The adapter registers machine outputs as bindings with bitmask dirty tracking. Zag handles focus management, keyboard navigation, and ARIA attributes via its machines; LLui handles rendering via its DOM functions and `portal()` for floating content. Styling uses CSS targeting `data-state`, `data-part`, and `data-scope` attributes emitted by the machines. See 08 Ecosystem Integration §1 for the full specification, component anatomy, and open questions.
