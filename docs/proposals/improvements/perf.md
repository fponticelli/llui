...
6. ~~**Phase 3 of C** — precompiled-library row-factory ABI~~ — **evidence-closed (2026-06-10), do not build without a NEW trigger.** After migrating both real consumers to the published 0.11.x packages, a full tier-coverage scan (compiler `onLowerBail`/`onPerfDiagnostic` hooks + AST delegation analysis over every `each` site) found ZERO qualifying call sites:
   - **dicerun2** (75 sites: 17 direct / 21 arm / 4 signalEach / 25 verbatim): no row delegates to any external package. `@llui/components` is imported exactly once (the `sortable` connect-part — parts, not rows). The old corpus signal (`row-top-not-element` ×14 ≈ "cross-file delegation") resolved to IN-APP relative-import helpers (`rolled-result.ts` ×5, `param-controls.ts` ×5) on small lists (explore feed = 12/page, param rows = handfuls), mostly already arm-tier. The one big list (my-rolls, unpaginated library) delegates SAME-FILE to a helper that's structurally un-factorable (spreads another helper's return + nested `each` detail panel) — no ABI helps. 10 of the 25 verbatim are dice-roll's recursive tree (shape-divergent by design).
   - **dungeonlogs** (client 47 sites: 28 direct / 15 arm / 1 signalEach / 3 verbatim; `packages/ui` has zero `each` sites): cross-package row delegation EXISTS (`calendarInput`/`markdown`/`validityEditor` ← `@dl/ui`) but `@dl/ui` ships TS **source** (`exports` → `./src/index.ts`), so it compiles through the consumer's vite pipeline — structurally in-app cross-file, outside the precompiled-dist ABI. Those sites are arm-tier editor widgets / parse-dominated (`markdown()`) on small or bounded lists; the big lists (entity list, inbox) are inline-element arm-tier rows the ABI wouldn't touch.

   The ABI's precondition — a consumer rendering large create-hot lists through rows imported from a **precompiled** component package — exists in neither app. Revisit only if such a consumer appears; to re-verify, run `node packages/compiler/scripts/tier-coverage.mjs <src-root>` (build the compiler first) — per-file `transformSignalComponentSource` with both hooks + an AST scan of `each(items, {key, render})` render bodies for imported-call targets.
