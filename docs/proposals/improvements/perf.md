...
6. ~~**Phase 3 of C** — precompiled-library row-factory ABI~~ — **evidence-closed (2026-06-10), do not build without a NEW trigger.** After migrating both real consumers to the published 0.11.x packages, a full tier-coverage scan (compiler `onLowerBail`/`onPerfDiagnostic` hooks + AST delegation analysis over every `each` site) found ZERO qualifying call sites:
   ...
   The ABI's precondition — a consumer rendering large create-hot lists through rows imported from a **precompiled** component package — exists in neither app. Revisit only if such a consumer appears; to re-verify, rebuild the (untracked) `packages/compiler/tier-coverage.local.mjs` driver: per-file `transformSignalComponentSource` with both hooks + an AST scan of `each(items, {key, render})` render bodies for imported-call targets.
