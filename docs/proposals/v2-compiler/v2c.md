...
**Design decision (resolved).** The clean primitive is a `transformCall?(ctx, node) → node | null` hook on `CompilerModule`. The registry's run() now exposes a Phase 2b that dispatches matching modules per `CallExpression` and chains their results. Module ordering is load-bearing across rewrites that compose (e.g., memo-wrap before mask injection); declaration order determines who fires first. **Landed in v2c/decomp-11**: hook on the interface + Phase 2b in `ModuleRegistry.run()` + 7 unit tests covering dispatch, chain, ordering reversal, analysis-then-rewrite split, and zero-cost-when-absent. The phase is skipped (and the analysis.sourceFile reference is unchanged) when no active module declares `transformCall` — metadata-only module sets pay zero overhead.

The chosen suboption is (b.i): **migrate the inline call-rewrites to modules.** Bundle-size goldens (§7.9 step 5) become real once stripping `compiler-devtools` removes the dev instrumentation passes and stripping `compiler-ssr` removes hydration markers. Migration is incremental — each inline `try*` helper extracts to its own module, validated against existing tests, lands as a separate commit. The first POC target will be a small self-contained rewrite (likely `tryWrapEachItemsWithMemo` or `tryEmitRowFactory`); the design choices that surface there (e.g., per-target factory inputs vs accessor-resolver lookup) inform the rest.
