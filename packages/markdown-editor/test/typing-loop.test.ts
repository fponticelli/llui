...
// anything this test measures — 6.4s measured on a deliberately saturated
// machine, against ~1s idle. That used to carry its own `{ timeout: 30_000 }`;
// the same budget is now stated once, workspace-wide, in `vitest.shared.ts`
// (#147), and restating it here would only shadow it the next time it moves.
