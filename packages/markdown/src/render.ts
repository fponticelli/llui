...
export function markdown(source: Reactive<string>, opts: MarkdownOptions = {}): Mountable {
  const options = resolveOptions(opts)

  if (!isSignal(source)) {
    const root = parseMarkdown(source, opts)
    const ctx = makeContext(options, collectDefinitions(root))
  ...
  const units = source.map((src): RenderUnit[] => {
    ...
    const ctx = makeContext(options, collectDefinitions(root))
    return toKeyedBlocks(root, src, options).map((block) => ({
      key: block.key,
      hash: block.hash,
