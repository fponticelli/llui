...
describe('integration: mint → ws → describe → state', () => {
  ...
  // ── User-input roundtrip ────────────────────────────────────────────
  // Single test exercising the entire inverted-input chain over real
  // sockets and a real HTTP server: composer.submitUserInput →
  // user-input-submitted WS frame → pairing registry buffer →
  // /lap/v1/wait-for-user-input handler → JSON response. Catches
  // wiring bugs that no per-layer unit test could (e.g. a frame schema
  // drift between client/server, a missing router case, a missed
  // Content-Type header).
  it('user-input roundtrip: client.submitUserInput → wait_for_user_input LAP returns submitted', async () => {
    const mintRes = await fetch(`${baseUrl()}/agent/mint`, { method: 'POST' })
    const { token } = (await mintRes.json()) as MintResponse

    const ws = new WebSocket(`${wsUrl('/agent/ws')}?token=${encodeURIComponent(token)}`)
    const wsClient = attachWsClient(
      ws as unknown as import('../../src/client/ws-client.js').WsLike,
      makeFakeRpcHost({}),
      makeHelloBuilder('ChatApp'),
    )
    ...
    // Hello + register settle.
    await new Promise((r) => setTimeout(r, 50))

    // Park the LAP wait FIRST so the registry has a parked waiter when
    // the frame arrives. (We test the buffered-then-drained flow as a
    // separate case.)
    const waitPromise = fetch(`${baseUrl()}/agent/lap/v1/wait-for-user-input`, {
      method: 'POST',
      headers: { authorization: `Bearer ${token}`, 'content-type': 'application/json' },
      body: JSON.stringify({ timeoutMs: 5_000 }),
    })

    // Give the server a moment to process the request and park a waiter.
    await new Promise((r) => setTimeout(r, 30))

    // User submits via the in-app composer → ws-client emits the upstream
    // frame + the synthesized log-append entry.
    wsClient.submitUserInput('hello from the user', 1700)
    ...
    expect(body.text).toBe('hello from the user')
    expect(body.at).toBe(1700)

    ws.close()
    await new Promise<void>((resolve) => ws.once('close', resolve))
  })

  it('user-input roundtrip: messages buffer when no waiter is parked, drain on next LAP call', async () => {
    const mintRes = await fetch(`${baseUrl()}/agent/mint`, { method: 'POST' })
    const { token } = (await mintRes.json()) as MintResponse
    ...
    // User types BEFORE the agent reaches the wait tool. Two messages
    // fan in; both should be buffered.
    wsClient.submitUserInput('first', 100)
    wsClient.submitUserInput('second', 200)

    // Give the server a moment to process the inbound frames.
    await new Promise((r) => setTimeout(r, 30))

    // First wait drains 'first'.
    const w1 = await (
      await fetch(`${baseUrl()}/agent/lap/v1/wait-for-user-input`, {
        method: 'POST',
        headers: { authorization: `Bearer ${token}`, 'content-type': 'application/json' },
        body: JSON.stringify({ timeoutMs: 1_000 }),
      })
    ).json()
    expect(w1).toEqual({ status: 'submitted', text: 'first', at: 100 })

    // Second wait drains 'second'.
    const w2 = await (
      await fetch(`${baseUrl()}/agent/lap/v1/wait-for-user-input`, {
        method: 'POST',
        headers: { authorization: `Bearer ${token}`, 'content-type': 'application/json' },
    ...
    expect(w2).toEqual({ status: 'submitted', text: 'second', at: 200 })

    ws.close()
    await new Promise<void>((resolve) => ws.once('close', resolve))
  })

  it('user-input roundtrip: returns timeout when no input arrives within the deadline', async () => {
    const mintRes = await fetch(`${baseUrl()}/agent/mint`, { method: 'POST' })
    const { token } = (await mintRes.json()) as MintResponse
    ...
    const res = await fetch(`${baseUrl()}/agent/lap/v1/wait-for-user-input`, {
      method: 'POST',
      headers: { authorization: `Bearer ${token}`, 'content-type': 'application/json' },
