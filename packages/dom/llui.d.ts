...
export interface AppHandle { dispose(): void; flush(): void }

export declare function component<S, M, E>(def: ComponentDef<S, M, E>): ComponentDef<S, M, E>
export declare function mountApp<S, M, E>(container: HTMLElement, def: ComponentDef<S, M, E>, data?: unknown): AppHandle
...
export declare function each<S, T>(opts: {
  items: (s: S) => T[]
...
export declare function foreign<S, T extends Record<string, unknown>, Instance>(opts: {
  ...
  props: (s: S) => T
  sync: ((inst: Instance, props: T, prev: T | undefined) => void) | {
...
export declare function button(props?: Record<string, unknown>, children?: Node[]): HTMLButtonElement
...
export declare function select(props?: Record<string, unknown>, children?: Node[]): HTMLSelectElement
export declare function option(props?: Record<string, unknown>, children?: Node[]): HTMLOptionElement
...
export declare function http(opts: { url: string; method?: string; body?: unknown; headers?: Record<string, string>; onSuccess: string; onError: string }): { type: 'http' }
...
export declare function debounce(key: string, ms: number, inner: { type: string }): { type: 'debounce' }
...
export declare function handleEffects<E extends { type: string }>(): {
  else(handler: (effect: E, send: Send<unknown>, signal: AbortSignal) => void): (effect: E, send: Send<unknown>, signal: AbortSignal) => void
}

// ── @llui/test ────────────────────────────────────────────────────

export declare function testComponent<S, M, E>(def: ComponentDef<S, M, E>, data?: unknown): {
  state: S; effects: E[]; allEffects: E[]
  history: Array<{ prevState: S; msg: M; nextState: S; effects: E[] }>
  send(msg: M): void; sendAll(msgs: M[]): S
}
export declare function testView<S, M, E>(def: ComponentDef<S, M, E>, state: S): {
