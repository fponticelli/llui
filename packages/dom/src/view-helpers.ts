 ...
 *
 * Note: calls through `h` currently fall back to `FULL_MASK` in the runtime
 * because the Vite plugin's mask-injection pass only rewrites bare
 * identifier calls (`show(...)`). This is correctness-preserving but loses
 * per-binding gating. Compiler support for member-expression calls is a
 * follow-up.
 */
 ...
 * of parent state.
 */
function createSlicedView<Root, Sub, M>(
  send: Send<M>,
  lift: (r: Root) => Sub,
): View<Sub, M> {
  return {
