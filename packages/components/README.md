...
- **Select-all is reached through the header.** Pass `selectAllColumnId` to `connect()` (or to `dataTable.connect()`) naming the column whose `columnheader` hosts the checkbox, and Enter/Space on that focused header sends `toggleAll`. Without it the select-all is mouse-only.
...
- **An `outside` registration leaks page-wide.** The lookup carries no nesting information, so any live `outside` registration hides interactions inside it from _every_ open layer, not just the one it is nested in. Concretely: an unrelated `tooltip({ closeOnEscape: false })` — the only stock config that reaches the `outside` aspect — open anywhere on the page will suppress an open `select`'s outside-dismissal for clicks inside that tooltip. It needs a non-default config _and_ a simultaneously-open layer, so it is narrow, but it is the price of covering a layerless overlay at all.
