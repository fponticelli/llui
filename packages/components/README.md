...
**Registration is per-aspect.** A lookup is global — it cannot tell which layer asked — so one answer is not right for all three consumers at once. Two sibling popovers both want `focus` and `hide`, but a click inside the lower one must still dismiss the upper one; registering the lower for `outside` too would make that click read as "inside a nested layer" and the upper would stay open. So a registration names the aspects it participates in — `outside` (`dismissable`), `focus` (`focus-trap`), `hide` (`aria-hidden`) — defaulting to all three. (A modal surface's inner overlays are a different mechanism: a modal never registers at all, so nothing about it depends on the aspect list.)
...
**Two limits of a flat registry.** Both are deliberate; write them down rather than be surprised by them.

- **An `outside` registration leaks page-wide.** The lookup carries no nesting information, so any live `outside` registration hides interactions inside it from _every_ open layer, not just the one it is nested in. Concretely: an unrelated `tooltip({ closeOnEscape: false })` — the one _overlay-engine_ config that reaches the `outside` aspect — open anywhere on the page will suppress an open `select`'s outside-dismissal for clicks inside that tooltip. That one needs a non-default config _and_ a simultaneously-open layer, so it is narrow. **Hand-written registrations are not narrow**: `registerNestedLayer` defaults to all three aspects, so any caller that omits `aspects` takes `outside` too — `@llui/markdown-editor`'s floating overlays do exactly that on their _default_ config. Either way it is the price of covering a layerless overlay at all; pass an explicit `aspects` list wherever the dismissable stack already orders the surface.
