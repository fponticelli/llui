...
- APG grid keyboard nav (arrows, Home/End, Ctrl+Home/End, PageUp/Down, Space to select the row, Ctrl/Cmd+A to select all, Enter to activate) with a single roving tab stop; works with rows rendered via each or virtualEach.
- The checkbox parts carry `tabindex="-1"` by design. A `role="grid"` has exactly ONE tab stop, so every selection control is operated from the roving cell — Space for the row, Ctrl/Cmd+A for select-all (whose `columnheader` is not in the roving sequence). Their Space handlers still apply when the checkbox is focused programmatically.
