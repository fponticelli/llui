 ...
 *
 * The slots are FUSED — `border-y border-e` with `first:border-s` — so the group
 * reads as one control in either writing direction.
 *
 ...
 * inputs. Both are bound rather than picking one, so the upstream interaction
 * classes remain present for anyone pasting a shadcn snippet, while logical
 * border/radius utilities make the fused slots work in either direction. The
 * LLui machine lights the same ring through `focus-visible:` with no extra wiring.
 */
