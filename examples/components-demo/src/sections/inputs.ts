...
const init = (): [State, never[]] => [
  {
    ...
    togGroup: toggleGroup.init({ items: ['bold', 'italic', 'underline'], value: ['bold'], type: 'multiple' }),
...
const update = mergeHandlers<State, Msg, never>(
  ...
  sliceHandler({ get: (s) => s.pinInput, set: (s, v) => ({ ...s, pinInput: v }), narrow: (m) => (m.type === 'pinInput' ? m.msg : null), sub: pinInput.update }),
  sliceHandler({ get: (s) => s.tagsInput, set: (s, v) => ({ ...s, tagsInput: v }), narrow: (m) => (m.type === 'tagsInput' ? m.msg : null), sub: tagsInput.update }),
...
const App = component<State, Msg, never>({
  ...
  update,
  view: (send) => {
    const sw = switchMachine.connect<State>((s) => s.switch, (m) => send({ type: 'switch', msg: m }))
    const cb = checkbox.connect<State>((s) => s.checkbox, (m) => send({ type: 'checkbox', msg: m }))
    const rg = radioGroup.connect<State>((s) => s.radio, (m) => send({ type: 'radio', msg: m }), { id: 'radio-demo' })
    const tg = toggleGroup.connect<State>((s) => s.togGroup, (m) => send({ type: 'togGroup', msg: m }))
    const ni = numberInput.connect<State>((s) => s.number, (m) => send({ type: 'number', msg: m }))
    const pw = passwordInput.connect<State>((s) => s.password, (m) => send({ type: 'password', msg: m }))
    const pin = pinInput.connect<State>((s) => s.pinInput, (m) => send({ type: 'pinInput', msg: m }), { id: 'pin-demo' })
    const ti = tagsInput.connect<State>((s) => s.tagsInput, (m) => send({ type: 'tagsInput', msg: m }))
    const ra = ratingGroup.connect<State>((s) => s.rating, (m) => send({ type: 'rating', msg: m }), { label: 'Rate' })
    const sl = slider.connect<State>((s) => s.slider, (m) => send({ type: 'slider', msg: m }))
    const pr = progress.connect<State>((s) => s.progress, (m) => send({ type: 'progress', msg: m }))

    // Pin input focus advance
    onMount(() => {
      const inputs = document.querySelectorAll<HTMLInputElement>('[data-scope="pin-input"][data-part="input"]')
    ...
    onMount(() => {
      ...
      const onMove = (e: PointerEvent): void => {
        if (dragging) send({ type: 'slider', msg: { type: 'setThumb', index: 0, value: compute(e.clientX) } })
      }
    ...
    const progressBtn = (txt: string, v: number | null): Node =>
      button({ class: 'btn btn-secondary text-xs', onClick: () => send({ type: 'progress', msg: { type: 'setValue', value: v } }) }, [text(txt)])
      ...
      sectionGroup('Form controls', [
        card('Switch', [
          label({ class: 'flex items-center gap-3' }, [
            ...
            span({ class: 'text-sm' }, [text((s: State) => (s.switch.checked ? 'Notifications on' : 'Notifications off'))]),
          ]),
        ]),
        card('Checkbox', [
          label({ class: 'flex items-center gap-3' }, [
            div({ ...cb.root, class: 'cb' }, [
              span({ ...cb.indicator, class: 'cb__indicator' }, [text((s: State) => (s.checkbox.checked === true ? '✓' : s.checkbox.checked === 'indeterminate' ? '−' : ''))]),
            ]),
            span({ class: 'text-sm' }, [text((s: State) => (s.checkbox.checked === true ? 'Checked' : s.checkbox.checked === 'indeterminate' ? 'Indeterminate' : 'Unchecked'))]),
          ]),
          div({ class: 'mt-3 flex gap-2' }, [
            button({ class: 'btn btn-secondary text-xs', onClick: () => send({ type: 'checkbox', msg: { type: 'setChecked', checked: 'indeterminate' } }) }, [text('Set indeterminate')]),
          ]),
        ]),
        card('Radio Group', [
          div({ ...rg.root, class: 'flex flex-col gap-2' },
            ['small', 'medium', 'large'].map((v) => {
              const p = rg.item(v)
              return label({ class: 'flex items-center gap-2 cursor-pointer' }, [
                div({ ...p.root, class: 'radio' }, [div({ ...p.indicator, class: 'radio__indicator' }, [])]),
                span({ class: 'text-sm' }, [text(v.charAt(0).toUpperCase() + v.slice(1))]),
              ])
            }),
          ),
          div({ class: 'mt-3 text-sm text-slate-600' }, [text('Size: '), text((s: State) => s.radio.value ?? 'none')]),
        ]),
        card('Toggle Group', [
          div({ ...tg.root, class: 'inline-flex rounded-md border border-slate-300 overflow-hidden' },
            ['bold', 'italic', 'underline'].map((v) =>
              button({ ...tg.item(v).root, class: 'tg-item' }, [text(v.charAt(0).toUpperCase())]),
            ),
          ),
          div({ class: 'mt-3 text-sm text-slate-600' }, [text('Active: '), text((s: State) => (s.togGroup.value.length > 0 ? s.togGroup.value.join(', ') : 'none'))]),
        ]),
        card('Number Input', [
          ...
          div({ class: 'mt-3 text-sm text-slate-600' }, [text('Quantity: '), text((s: State) => String(s.number.value ?? 0))]),
        ]),
        card('Password Input', [
          div({ ...pw.root, class: 'pw-root' }, [
            input({ ...pw.input, class: 'pw-input' }),
            button({ ...pw.visibilityTrigger, class: 'pw-toggle' }, [text((s: State) => (s.password.visible ? 'Hide' : 'Show'))]),
          ]),
        ]),
        card('Pin Input (OTP)', [
          ...
          div({ class: 'mt-3 text-sm text-slate-600' }, [text('Code: '), text((s: State) => s.pinInput.values.join('') || '(empty)')]),
        ]),
        card('Tags Input', [
              ...
              ...each<State, string, TagsInputMsg>({
                ...
                render: ({ item, index }) => {
                  ...
                  return [
                    span({ class: 'tag' }, [
                      text(tagFn),
                      button({ type: 'button', class: 'tag-x', onClick: () => send({ type: 'tagsInput', msg: { type: 'removeTag', index: index() } }), 'aria-label': 'Remove tag' }, [text('×')]),
                    ]),
                  ]
                },
              }),
        ...
        card('Rating', [
          ...
          div({ class: 'mt-3 text-sm text-slate-600' }, [text('Rating: '), text((s: State) => String(s.rating.value)), text(' / '), text((s: State) => String(s.rating.count))]),
        ]),
        card('Slider', [
          div({ ...sl.root, class: 'relative' }, [
            div({ ...sl.control }, [div({ ...sl.track }, [div({ ...sl.range }, [])]), div({ ...sl.thumb(0).thumb }, [])]),
          ]),
          div({ class: 'mt-2 text-sm text-slate-600' }, [text('Value: '), text((s: State) => String(s.slider.value[0] ?? 0))]),
        ]),
        card('Progress', [
          div({ ...pr.root, 'aria-label': 'Upload progress' }, [div({ ...pr.track }, [div({ ...pr.range }, [])])]),
          div({ class: 'mt-2 text-sm text-slate-600' }, [text((s: State) => pr.valueText(s))]),
          div({ class: 'mt-3 flex gap-2' }, [progressBtn('25%', 25), progressBtn('65%', 65), progressBtn('100%', 100), progressBtn('Indeterminate', null)]),
        ]),
      ]),
    ]
  },
