...
export const router = createRouter<Route>([
  route([], () => ({ page: 'search', q: '', p: 0, data: { type: 'idle' } })),
  route(['search'], { query: ['q', 'p'] }, ({ q, p }) => ({ page: 'search', q: q ?? '', p: p ? parseInt(p, 10) : 0, data: { type: 'loading' } })),
  ...
  mode: 'history',
  fallback: { page: 'search', q: '', p: 0, data: { type: 'idle' } },
})
