...
- **what changed** → root `CHANGELOG.md` first (including Unreleased), then the `version` field
  of `packages/<pkg>/package.json` across git history and the actual package diffs for exact,
  structured evidence. Release commits, not tags, anchor published batches.
...
root** (`/Users/Shared/Projects/llui`). All paths below are relative
to that root.
...
- **CHANGELOG is the overview, diffs are the proof.** Read root `CHANGELOG.md`, then use
  `changelog.mjs`, which anchors on the manifest `version` field across history rather than tags.
  If it prints "no commit sets the manifest to X," the pinned
