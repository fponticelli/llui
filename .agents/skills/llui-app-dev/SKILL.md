...
description: >-
  ...
  @llui/* or the llui-agent bridge. This includes core authoring, compiler/Vite
  diagnostics, headless components and interactions, effects, routing, transitions,
  tests, SSR, Markdown/editors, annotations, A2UI, and agent surfaces. Load it before
  answering, not after.
...
```ts
import { component, mountApp, div, button, text, show, each, mapSend } from '@llui/dom'
```
...
### 12. Non-finite numbers in state → JSON changes their meaning

`NaN` and `±Infinity` serialize as `null`, while negative zero is easy to lose across
normalization. This is especially dangerous in numeric component state and route identity.

- **Tell:** arithmetic or DOM measurements are written directly into state; an unbounded
  minimum is represented as `-Infinity`; a route codec accepts a non-finite number.
- **Fix:** reject or normalize non-finite input at every ingress. Spell an absent bound by
  omitting it (`min?: number`) rather than with an infinity sentinel. Current
  `@llui/components` numeric machines and `@llui/router` route locations enforce this;
  custom state and reducers must preserve the same invariant.
...
5. Reach for `@llui/components` before hand-rolling interactive UI (dialogs, menus, selects, tabs, trees, forms); use `@llui/interactions` for a custom interaction primitive without a component state machine; reach for `@llui/effects` builders before hand-rolling fetch/debounce/cancel.
