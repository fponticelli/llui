...
Install `@llui/a2ui`, `@llui/dom`, and `@llui/interactions` together. The two peers must
resolve once so A2UI's component-backed controls share the host's signal and interaction
registries.
...
- **Reconnects resume, they do not mint around revocation:** the server can resurrect an
  evicted MCP session while its authenticated pairing remains valid, but closed buffers are
  bounded and reclaimed. Treat a refused or revoked token as terminal; never loop by minting
  replacement credentials automatically.
