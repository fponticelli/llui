...
- `imagePlugin({ resolveSrc })` maps a portable stored image path to the URL rendered in the
  DOM without changing node data or emitted Markdown. The stored URL is sanitized before the
  resolver runs; the result is deliberately not re-checked so desktop/private schemes work.
...
- **Controlled echo has one authority:** write through `setValue` (or the
  `ForeignController.applyValue` seam), not directly through the raw Lexical editor. The seam
  suppresses document-equivalent inbound and outbound commits so the caret survives.
- **Custom plugins use the commit hub:** current `PluginContext` includes `onCommit` and
  `withFacts`; do not hand-roll `{ emit }` or register another update listener for selection
  overlays. Call `createCommitHub(editor, emit)` in custom hosts/tests.
...
`@llui/lexical-loro` is the alternative Loro binding (`loroCollab`). Text formats are
independent named marks rather than a combined bitmask, and custom `TextNode` subclasses use
the JSON carrier path so their extra fields survive sync. Its deep accumulated-history test is
a separate stress lane; ordinary app tests should not copy that workload.
