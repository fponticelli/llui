...
## Singleton framework packages are peer dependencies — CRITICAL
...
Apply the same peer + dev pattern to `@llui/interactions` when the package consumes its shared
document-level dismissal, focus, nested-layer, or scroll/ARIA registries. Two installed copies
split overlay ownership just as two DOM copies split render context.
...
3. **`site/src/generate-api.ts`** — auto-generates `site/content/api/<slug>.md` from
   ...
   `pnpm --dir site generate` and commit the generated `content/api/<slug>.md` and llms outputs — the
...
pnpm --filter @llui/<name> build check test lint
pnpm turbo build --filter=!@llui/site   # CI's build scope
pnpm turbo check                       # unfiltered: includes @llui/site
pnpm turbo lint                        # unfiltered: includes @llui/site
