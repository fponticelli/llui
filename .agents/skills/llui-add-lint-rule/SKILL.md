...
## Parsed-module entry points + ScriptKind (REQUIRED)

The public signal entry is `lintSignalSource(mod: ParsedModule)`. Parse through one shared
`createModuleCache()` and call `modules.get(realFilename, source)`; the cache selects the correct
`ScriptKind` from the filename. **Always retain the real id/filename** — parsing a `.ts` file as
TSX misparses generic-arrow syntax (`const id = <T>(x: T) => x`) as JSX and can fire spurious
signal errors. The same parsed-module seam is used by `lintAnnotationSyntaxSource(mod)` and
`lintTagSendSource(mod)`, which cover sibling and non-component modules.

## Non-bypassable build error (vite-plugin)

`packages/vite-plugin/src/compile-plugin.ts` creates one module cache per transform, calls
`lintSignalSource(modules.get(cleanId, code))`, and splits diagnostics:
...
**A new signal correctness rule needs NO vite-plugin change — it blocks automatically.** Only add
vite-plugin handling if you intend a silent auto-fix path (mirror the `convention` handling).
If the rule must also inspect imported sibling modules or files outside the component filename
convention, wire the appropriate parsed-module entry point explicitly and test both paths.
...
`agent-annotation-syntax`, `tag-send-drift`.

Before reporting coverage, apply and inspect a **faithful mutation** that changes only the rule
property under test, prove the diff reached live code, and record which tests kill or survive it
and why. Re-run the mutation against the final tree.

Finish with `pnpm --filter @llui/compiler build check test`,
`pnpm --filter @llui/vite-plugin build check test`, then a smoke build of an example
