...
8. **Publish each built scope** with `buildAndPublishScope(built)` (`scope-build.ts`).
   ...
   Treat child-scope traversal as a snapshot: a scope created while its parent is being traversed
   must not be traversed until the next outer traversal. Never splice a freshly published child
   into the active loop and continue into it; that can reconcile a just-built subtree twice.
...
Before reporting coverage, apply and inspect faithful mutations for the ownership/traversal
properties, prove each diff reached live code, and record which tests kill or survive each one
and why. Re-run the table against the final tree.
