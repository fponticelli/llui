...
- **Unrelated to that change but worth checking while you are in the file: your own base-token overrides are probably dead on a dark OS.** `tokens-dark.css`'s media guard is `:root:not([data-theme='light']):not(.light)` — specificity **(0,3,0)** — and a `.dark` / `[data-theme='dark']` block is **(0,1,0)**, so on a dark OS the library's block outranks yours and replaces your token with the library default, for EVERY preference and regardless of source order; under `'system'` your selector additionally matches nothing at all. That is [#241](https://github.com/fponticelli/llui/issues/241), it predates this release, and the workaround is a twin block under the same guard: `@media (prefers-color-scheme: dark) { :root:not([data-theme='light']):not(.light) { … } }`. Separately, a shadcn theme generator's output scopes everything under `.dark` alone and **nothing in `@llui/components` ever writes that class**, so pasted generator output never matches in any cell — [#242](https://github.com/fponticelli/llui/issues/242); add `[data-theme='dark']` to the selector. `components-demo` had both gaps. The derived `color-mix()` tokens need no twin.
...
- **Fixed** `components-demo`'s brand override defined `--primary` only under `.dark, [data-theme='dark']`, which the library's own `(0,3,0)` media guard outranks on any dark OS — so the demo rendered shadcn's default dark `--primary`, not its brand colour, under BOTH the `'dark'` and `'system'` preferences. Pre-existing and unrelated to the `applyTheme` change (#241); it now carries the twin block under the same guard. `dashboard` needed no CSS change: its stylesheet is dark-first and already answers the no-attribute case.

### Docs

- **Added** `site/content/styling.md` documents that `applyTheme` publishes the PREFERENCE (so `'system'` removes the attribute), and gains a section on overriding a base token on a dark OS: the library's media guard wins on SPECIFICITY for every preference (#241), a shadcn generator's `.dark`-only output matches nothing because nothing writes that class (#242), and the twin block that works around both.
