...
A local-state ladder for widgets (`island()`, `constant()`), reference ranges for `meter`, four compiler rules that were deciding by identifier NAME instead of import provenance, and two published `.d.ts` a strict consumer could not compile.

Eighteen of the twenty-one bumps are cascade-only: `workspace:^` publishes as `^0.13.3`, which excludes `0.14.0`, so every dependent of `@llui/dom` / `@llui/compiler` / `@llui/components` republishes to keep its peer range satisfiable. `@llui/cli`, `@llui/effects`, `@llui/notes-format` and `@llui/security` depend on none of the three and are unchanged.

### Breaking

- **`@llui/dom` — `subApp()` is now `island()`, on the main barrel.** `@llui/dom/escape-hatch` still exports `subApp` as a deprecated alias, so nothing breaks today, but the framing was the defect ([#231](https://github.com/fponticelli/llui/issues/231)): a subpath called `escape-hatch`, a module header calling it an anti-pattern and a MANDATORY `reason` field all told an author that local widget state was something to avoid. `island()` is the middle rung of a three-tier ladder — T1 static `connect(constant(v), noSend, opts)`, T2 local `island({ def })`, T3 hoisted `connect(state.at('x'), send)` — and its `reason` is optional. It also returns a single `Mountable` rather than an array, so the spread at the call site goes away.
- **`@llui/dom` — anonymous head keys are namespaced per owning instance** ([#240](https://github.com/fponticelli/llui/issues/240)). A `style`/`script` with no `id`/`src` has nothing to dedup on, so it is keyed by an ordinal — and that ordinal was per BUILD. Every isolated instance starts a fresh build, so an `island` minted `style:#1`, the key its host already held, and one entry silently overwrote the other. Two fresh counters agree with each other and are still wrong. Keys now read `style:#~1/1` for the first island placed in a build, nesting as `~1/~2`; the namespace is allocated at PLACEMENT, the one point server render and client mount agree on. If you assert on emitted head keys, they have changed shape.
- **`@llui/dom` — an `island()` or `lazy()` among an `each` row's top-level nodes is REJECTED on the client**, as it always was on the server ([#239](https://github.com/fponticelli/llui/issues/239)). Breaking only for code that was already corrupting; see the `@llui/dom` section for why the client half stayed silent.
- **`@llui/compiler` — a bare `state` in an `each` row is no longer rebased onto component state unless it PROVABLY denotes it** ([#247](https://github.com/fponticelli/llui/issues/247)). A view helper's parameter named `state` — the composition pattern the docs recommend — was silently rebased onto the app root, so `plot(state.at('share'), …)` emitted `deps: ['state.activeIndex']` reading a different object than the author wrote. Clean build, clean type-check, wrong render, and the verdict turned purely on the SPELLING. Three `each` sites in this repo's own tests lose the direct tier as a result; none is a production authoring shape.
- **`@llui/components` `meter` — `MeterThreshold` and `thresholdState` are gone** ([#235](https://github.com/fponticelli/llui/issues/235)), replaced by `MeterTone = 'optimal' | 'suboptimal' | 'critical' | 'neutral'` with `bandAt` / `toneAt`. `MeterState` now carries ONE quality model, `bands: MeterBand[]`; `low`/`high`/`optimum` become INIT OPTIONS that compile into three positional segments. Nothing reads `state.low` any more. One boundary MOVED: `value === high` now reads as the upper segment rather than the middle one, because edges are uniformly half-open. `neutral` is the ABSENCE of an opinion and is deliberately not a synonym for `optimal` — `init()` with no thresholds used to answer `'optimal'`, i.e. a plain disk gauge claiming the reading was good.
...
- **`subApp()` → `island()`.** Import from `@llui/dom` instead of `@llui/dom/escape-hatch`, drop the spread (it returns one `Mountable` now), and drop `reason` unless you want it. The alias keeps working.
- **Wrap an `island()`/`lazy()` used as an `each` row root:** `li([island({ def })])`. It applies however many siblings the anchor has — `[div(…), island({ def })]` loses the body the same way. `virtualEach` is unaffected.
- **`meter`:** replace `thresholdState(state)` with `toneAt(state, value)` and read `MeterTone`. Keep passing `low`/`high`/`optimum` to `init` if that is your model — it still compiles into bands. To express a real reference range, pass `bands` directly; edges are `[from, to)`, first match in author order, with the top of the track inclusive. A value in no band reports no `data-band` and no name in `aria-valuetext` — deliberately, because inventing membership misstates a lab result.
- **If you have a view helper whose parameter is named `state`,** its `each` rows now compile correctly rather than against the app root. No action needed; if you had worked around the old behaviour by renaming the parameter, you can rename it back.
- Calling `applyTheme(resolveTheme(pref))`? Drop the resolve and pass the preference: `applyTheme(pref)`. That removes the attribute for `'system'`, which is the state `tokens-dark.css` already answers in CSS — so `watchSystemTheme` is no longer needed to follow the OS, and SSR gains a spelling it can render. Behaviour is otherwise identical: the reference cell-by-cell rendering is unchanged in all six (preference × OS) combinations.
- **Your own base-token overrides now win in dark mode — and a `:root`-only override now leaks into it, where it used to be overruled on a dark OS.** `tokens-dark.css`'s media guard is now `:root:where(:not([data-theme='light'])):where(:not(.light))` — specificity **(0,1,0)** instead of **(0,3,0)** ([#241](https://github.com/fponticelli/llui/issues/241)). It used to outrank every consumer override of a base token on a dark OS, including `[data-theme='dark']` under an explicit `'dark'` preference, where the selector matches perfectly well and still lost. Two things to check in your own theme:
  - **A surface token overridden in `:root` alone now reaches dark mode in every cell.** Before, it leaked on a light OS and was overruled on a dark one. If you pinned a light surface in `:root` and let the library's dark value win the rest, that no longer happens — override it in the dark block too, or its paired `--*-foreground` will be the library's dark one over your light surface. Measured on `components-demo`'s own `--destructive`: 6.207:1 before, **3.750:1** after (below the 4.5 AA floor), 6.199:1 once the dark twin listed it.
  - **The `'system'` preference on a dark OS still needs a twin block**, and no specificity change can alter that: `applyTheme` writes no attribute for `'system'` and this package writes no `.dark` class, so there is nothing for your selector to match. Give the block a twin under the same guard: `@media (prefers-color-scheme: dark) { :root:where(:not([data-theme='light'])):where(:not(.light)) { … } }`. The derived `color-mix()` tokens need no twin.
- **`.dark` is a consumer convention, and `applyTheme` will not write it** ([#242](https://github.com/fponticelli/llui/issues/242), decided). `tokens-dark.css` keeps the `.dark` selector — `next-themes` and shadcn tooling write that class, and dropping it would take dark mode away from those consumers — but nothing in `@llui/components` adds it, and the file's header no longer implies otherwise. A shadcn theme generator's output scopes everything under `.dark` alone, so it needs one find-and-replace: add `[data-theme='dark']` to the selector. Maintaining the class in `applyTheme` was rejected on measurement — `.dark` names a _resolved_ theme, so it would reintroduce the JS `prefers-color-scheme` resolve #233 removed, and a class that goes stale on an OS flip does not merely fail to help, it suppresses the media query that was correct with no JS at all (with the preference on `'system'`: a stale `.dark` renders dark on a light OS, a stale `.light` renders light on a dark one).
...
- **Added** `island({ def, props, onProps, contexts, reason? })` on the main barrel — the T2 rung of the widget state ladder ([#231](https://github.com/fponticelli/llui/issues/231)). `props` is a DECLARATIVE channel built as a binding spec the way `foreign()`'s declared inputs are, so it lives in the HOST's scope and is mask-gated like any other binding, and the change arrives at the island as a MESSAGE (`onProps`) rather than as a poke at its state. Messages come back out through `onHandle`. Measured at N=500 leaves in jsdom: ~2.4x at mount (22.0 ms → 53.3 ms) and ~2x CHEAPER over 50 host updates (2.19 ms → 1.14 ms), because an island is not a child scope the host reconciler walks — mount cost bought for update isolation.
- **Added** `constant(value)` and `noSend` — a `Signal` and a `Send` for a widget whose values are FIXED for the life of the node, so `connect(constant(state), noSend, { id })` resolves a full part bag with no TEA runtime per instance. All 72 `connect()` entry points in `@llui/components` demand `Signal<S>` + `Send<M>`, which previously forced a hoisted state slice per static badge or gauge. Two traps this closes, both of which used to compile: `pathHandle(() => ({ v }), 'v')` renders EMPTY STRING while `peek()` reports the right value, and `Send<never>` — the intuitive "accepts no messages" spelling — is exactly backwards under `strictFunctionTypes`, since only `unknown` is assignable to every `Send<M>`.
- **Added** an isolated instance now RENDERS ON THE SERVER. `island()` used to emit a bare anchor comment under SSR, so the whole subtree was missing from `renderToString` and every island was a post-hydration pop-in — absent entirely without JS. It now runs `def.init()` for a seed (discarding its effects), builds, and mounts once against that seed. Its body reflects `init()`, NOT the first `props` value: a prop is a binding in the host's scope and is only resolvable once that scope reconciles.
- **Fixed** an isolated instance inherited NO ancestor context. `island`/`subApp` forwarded only the caller's explicit `contexts`, so `provide(Theme, 'PROVIDED', () => [...subApp({ def: Leaf })])` left `Leaf` reading `'DEFAULT'`. It fails silently and it is not cosmetic — `@llui/components` routes all i18n through `ComponentLocaleContext`, so every component mounted as an island fell back to default English. An explicit `contexts` map now merges OVER the inherited one rather than replacing it.
...
- **Fixed** (breaking for code that was already broken) an `island()` or `lazy()` among an `each` row's top-level nodes is now REJECTED on the client, as it always was on the server ([#239](https://github.com/fponticelli/llui/issues/239)). These primitives return an anchor comment and mount their real body as its **siblings**, so the body is not in the node list the row reconciler moves — but the comment is a perfectly stable node, which is why `each`'s existing `DocumentFragment` check could not see it. The client rendered happily and then corrupted on the **first reorder**: the anchors migrated and the mounted bodies stayed put, so a suite that never reorders stayed green while the identical source threw on the server. Both sides now fail identically and at mount. Measured before shipping: zero production call sites across `packages/`, `registry/`, `examples/` and `site/`. The fix is the wrap the docs already prescribed — `li([island({ def })])` — and it applies however many siblings the anchor has, since `[div(…), island({ def })]` loses the body the same way. `virtualEach` is unaffected: each of its rows lives in its own wrapper element, which travels with the row.
- **Improved** `each`'s stable-row-root diagnostic now has two branches. The old one named only `show`/`branch`/`each` and said "wrap the conditional body in an element", which sent an author who wrote no conditional hunting for one; the anchor branch now names `island`/`lazy` and explains that their body is mounted beside the anchor, and both branches end with the wrap spelled out for the primitive you actually used.

### `@llui/compiler@0.14.0`

- **Fixed** signal recognition was by identifier TEXT, not import provenance, and was wrong in both directions at once ([#238](https://github.com/fponticelli/llui/issues/238)). `isSignalExpr` tested `e.expression.text === 'derived'`, so a consumer's own `derived()` raised a build-stopping error on code that did nothing wrong, while `constant()` was not recognized at all — leaving `operator-on-signal` and `peek-in-slot` silently OFF for every constant. TypeScript covers the arithmetic and comparison directions on its own; template, ternary, logical and unary are covered by nothing, and the worst is `text(constant('') || 'FALLBACK')`, where a truthy handle makes `||` yield the HANDLE and the fallback is silently swallowed with no `[object Object]` tell. **This fixed a silent MISCOMPILE, not only diagnostics:** `each`'s direct tier treated an unrecognized `constant(…)` as static and emitted `String(constant(' · fixed'))`, measured against the real runtime as `[object Object]` with a clean build and a clean type-check. The fact is now stated once, resolved through the same binding gate that decides whether `div(…)` is the framework's, and it FAILS CLOSED.
- **Fixed** a static `each` row slot could read a FREE identifier the row factory never introduced, and stringify a `Signal` handle ([#244](https://github.com/fponticelli/llui/issues/244)). Recognition reads the factory CALL in an expression, so a handle arriving as a bare identifier named no call, fell into the static bucket, and rendered `[object Object]`: `li({ 'data-unit': unit }, [text(unit)])` emitted `applyAttr(_r0, "data-unit", unit)`. Both bindings reached it — a view-helper PARAMETER and a captured module-scope `const` — so it was the idiomatic composition shape rather than a corner case. Measured cost: **zero** tier losses and byte-identical transform output across 825 files / 85 gated in `packages/*/src`, `registry`, `examples` and `site/src`.
- **Fixed** `peek-in-slot` fired in REDUCER bodies — where `.peek()` is the documented correct one-shot read, and where the rule's own message tells you to put it ([#245](https://github.com/fponticelli/llui/issues/245)). The walk entered the FILE already believing it was in a reactive slot, so any `.peek()` anywhere in a module containing `component(` was reported as a non-bypassable build error. It now starts outside slot context and ENTERS at the positions that build reactive output, with a STICKY exit so an `on*` handler or a `.map`/`derived` body stays exempt from the inside — without which the fix reintroduces the very class it removes, measured on five shapes including the sanctioned block-body row idiom. The report set is a strict SUBSET of the old one for every program.
- **Fixed** an `each`'s dependency MASK went STALE when the row's `state` root was pruned ([#247](https://github.com/fponticelli/llui/issues/247)). Pruning alone ships a STALE ROW: the row's component-state reads fill `renderDeps`, which becomes the structural binding's `deps`, so a pruned read is invisible to the gate above the row and the DOM freezes at mount — correct row, wrong gate. Eight of nine row shapes go stale without this half.
- **Added** `imperative-dom-mutation` — a DOM write from a view's own event handler is now a build error ([#231](https://github.com/fponticelli/llui/issues/231)). A consumer reasoned from build-once views to the opposite conclusion ("the label is a static node, nothing reads it, so swapping `textContent` from the click handler is safe"); it is not, because `show`/`branch`/`each` may move, reuse or tear that node down on any later commit. The rule reads all three handler spellings — inline, `tagSend`-wrapped, and a module-scope NAME — because an inline-only rule fired on one of the four spellings it can be written in, and the two recoverable misses are the two most idiomatic here. `foreign()`, `island()` and `subApp()` are skipped whole, being the fixes the message names. Evidence: **0 reports** over 1531 files in `packages/`, `registry/`, `examples/` and `site/`; 21 faithful mutations, all killed.

### `@llui/components@0.19.0`

- **Added** `meter` reference ranges — N bands with a marker and a named `aria-valuetext` ([#235](https://github.com/fponticelli/llui/issues/235)). The old model held ONE optimum band, which cannot express a lipid panel (four bands), vitamin D (four) or a thyroid panel (three, asymmetric). `bands` is now the one quality model and the native `<meter>` spelling compiles into it. The accessibility win is the point: a bare `aria-valuenow` of 47 says nothing about whether the reading is in range, so an explicit band `label` is read out. Geometry is derived, never stored, and the `deriveOnce` cell lives in the `connect()` closure rather than at module scope — a page of static meters is the motivating case, and one shared cell would let two mounted meters evict each other on every update.
- **Added** a time-series `sparkline` with a reference band and calendar ticks ([#236](https://github.com/fponticelli/llui/issues/236)). `sparklineGeometry(points, opts)` is the load-bearing export — a pure function needing no signal, no send and no mount, so fifty in a table cost fifty calls. `utils/calendar-ticks.ts` is the substantive new capability: gridlines land on the 1st of the month, on midnight and on Jan 1, coarsening hour → day → week → month → quarter → year and then growing the year stride, O(1) at every rung.
- **Added** value-hued categorical `chip` rules to the baseline stylesheet ([#237](https://github.com/fponticelli/llui/issues/237)). The chip previously shipped only its registry (Tailwind) skin, so an app on `theme.css` got `chipHue()` and nothing to style with. No `.dark` override is needed — the mix toward `--background`/`--foreground` adapts by itself.
- **Fixed** four gaps in `theme.css`'s button recipe ([#234](https://github.com/fponticelli/llui/issues/234)): no disabled state, no height floor, no quiet variant, and only a filled destructive. `:disabled` / `[aria-disabled='true']` now dim to 0.5 and take `not-allowed`, and every variant's hover is gated on the same pair — the other half of `tokens.css`'s `button:not(:disabled) { cursor: pointer }`, which exists because Tailwind v4 dropped preflight's rule. NOTE: this takes each variant's hover selector from (0,2,0) to (0,4,0), so a downstream `.btn-primary:hover` override now needs matching specificity.
- **Fixed** `theme-switch`'s `applyTheme` could not express `'system'` (#233). It accepted only a RESOLVED theme and always wrote `data-theme`, so honouring the OS setting forced a `matchMedia` read and a pinned value in JS — strictly worse than doing nothing, because `tokens-dark.css` answers that case with `@media (prefers-color-scheme: dark) { :root:where(:not([data-theme='light'])):where(:not(.light)) }` and the attribute absent. Pinning made `watchSystemTheme` mandatory to track OS changes CSS follows for free, left SSR with no correct attribute to render, and destroyed the distinction between "the user chose dark" and "the OS is dark" in the one place that is readable from the DOM. (It does NOT cost a first-paint flash — `resolveTheme('system')` reads the same media query the first paint used, so the attribute it writes agrees with what is already on screen.) `resolveTheme`/`watchSystemTheme` are unchanged and stay for consumers who need the resolved value IN JS (canvas theming, `<meta name="theme-color">`) — they are simply off the critical path now. `connect()` needed no change.
...
- **Fixed** `clipboard`'s `copyToClipboard` DISCARDED `document.execCommand('copy')`'s boolean, so its `execCommand` fallback resolved on a refused write. That fallback is exactly what runs in an INSECURE CONTEXT — where `navigator.clipboard` is undefined, one of the three refusal cases #232 names — so the reducer fix alone left the announced-a-failed-copy bug reachable one branch down: the consumer dispatched `copied` from a resolved promise and the live region announced it. It now throws, so the documented "RESOLVES on success and REJECTS when the write is refused" contract holds on both branches.
- **Fixed** `clipboard`'s `connect({ onCopy })` was invoked with `''` instead of the value to write.
- **Fixed** `tokens-dark.css`'s `prefers-color-scheme` guard outranked every consumer override of a base token on a dark OS (#241). `:root:not([data-theme='light']):not(.light)` is specificity **(0,3,0)** — `:not()` takes its argument's specificity and two of them stack — while a consumer's `.dark` / `[data-theme='dark']` / `:root` block is **(0,1,0)**, so the library's own value won regardless of source order and regardless of preference. Measured in Chromium across all six (preference x OS) cells: with `--primary` overridden the LLui spelling lost in `os=dark pref=dark` and `os=dark pref=system`, and a shadcn generator's `.dark`-only block lost in every cell (#242, separate cause). The `:not()`s are now wrapped in `:where()`, which contributes zero, giving **(0,1,0)** — enough for a consumer block imported after this stylesheet to win on source order, and still enough to beat `tokens.css`'s own `:root`. Wrapping `:root` itself would be (0,0,0) and would lose to that `:root`, breaking dark mode outright; a test now pins the specificity relationship rather than the spelling, and a Chromium test measures the six-cell cascade. NOTE the behaviour this changes for a consumer: a `:root`-only override now reaches dark mode in every cell, where before it leaked on a light OS and was overruled on a dark one.
- **Decided** `applyTheme` will not maintain a `.dark` class (#242), and `tokens-dark.css` stops advertising one it never wrote. The selector STAYS — `next-themes` and shadcn tooling set that class themselves and dropping it would take dark mode away from those consumers (measured: with the class present, a generator-verbatim block wins in every cell once #241 lands) — but `.dark` names a RESOLVED theme, so maintaining it here would reintroduce the JS `prefers-color-scheme` resolve #233 removed. Measured with the preference on `'system'`: a stale `.dark` renders DARK on a light OS and a stale `.light` renders LIGHT on a dark one, because the class suppresses the media query that is correct with no JS at all. The `'system'`-on-dark-OS cell keeps the documented CSS twin block as its answer. Nothing in the repo used it, but it is the seam for performing the write from the trigger, which the fix above makes load-bearing — a consumer taking it would have copied an empty string.

### `@llui/lexical@0.5.1`

- **Fixed** `stripInternal` deleted `nodewidget.ts`'s entire `import` from the published `.d.ts`, shipping `LexicalNode`, `NodeKey`, `LexicalEditor`, `Klass` and `EditorDOMRenderConfig` as five unbound names — `TS2304` for any consumer building with `skipLibCheck: false` ([#253](https://github.com/fponticelli/llui/issues/253), [#254](https://github.com/fponticelli/llui/issues/254)). TypeScript's test for the internal tag is a raw substring search over every leading comment, so an 80-line `//` module header that merely MENTIONED the tag in prose deleted the statement below it. Almost nothing in-repo could see it: every workspace inherits `skipLibCheck: true`, so no package type-checks another's emitted types.

### `@llui/vike@0.13.1`

- **Fixed** every chain layer is its own build, so a layout's and its page's anonymous `<style>` collapsed onto one key and a three-layer chain kept ONE tag ([#240](https://github.com/fponticelli/llui/issues/240)). Each layer now names itself from its INDEX in the chain, never from mount order — a client navigation mounts only a suffix, which a counter would renumber.

### `@llui/vite-plugin@0.12.2`, `@llui/markdown-editor@0.8.6`, `@llui/devmode-annotate-editor@0.1.6`

- **Fixed** two published `.d.ts` a `skipLibCheck: false` consumer cannot compile ([#257](https://github.com/fponticelli/llui/issues/257)). `@llui/vite-plugin` leaked `import("rolldown").{TransformPluginContext,…}` into `dist/{compile,hud}-plugin.d.ts` — 6 × `TS2307` — through INFERENCE, not an import: the `create*Plugin` factories returned an object literal `satisfies Plugin`, so TypeScript wrote each hook's contextual `this` under the module where vite DECLARES it. The fix is a declared `: Plugin` return type, **not** a `rolldown` dependency: vite vendors and pins rolldown, so a hard dep installs a second copy whose types are a different identity. The public type surface is byte-unchanged. `@llui/markdown-editor` emitted a bare `import '@llui/markdown-editor/styles/editor.css'` into `@llui/devmode-annotate-editor`'s public entry (`TS2882`); its `./styles/*.css` subpaths now carry a `types` condition pointing at a shipped `export {}`. A sidecar `.d.ts` beside the CSS does NOT work under `moduleResolution: bundler` — with no `types` condition TypeScript never looks for one.

### All packages — build output

- **Improved** `pnpm check:dist` gained a fifth arm: every emitted `.d.ts` in a publishable package now TYPE-CHECKS in one `ts.Program` with `skipLibCheck: false` — the check a strict consumer actually runs against our published types ([#257](https://github.com/fponticelli/llui/issues/257)). The pre-existing binding arm cannot resolve module SPECIFIERS, which is exactly how the seven diagnostics above shipped under a green gate.
...
- **Fixed** `components-demo`'s brand override defined `--primary` only under `.dark, [data-theme='dark']`, which the library's own `(0,3,0)` media guard outranked on any dark OS — so the demo rendered shadcn's default dark `--primary`, not its brand colour, under BOTH the `'dark'` and `'system'` preferences. Pre-existing and unrelated to the `applyTheme` change (#241); it carries a twin block under the same guard, which after #241 is needed only for the `'system'` preference. That twin also now lists `--destructive`, and #241 is why it has to: the token is overridden in `:root`, and while the guard was (0,3,0) the library's dark value outranked that `:root` value in the `'system'`-on-dark-OS cell, so the pair resolved correctly by accident. At (0,1,0) the `:root` value wins and pins the LIGHT red under the library's dark near-black foreground — measured at 6.207:1 before, **3.750:1** without the line, 6.199:1 with it. `dashboard` and `registry-demo` needed no CSS change: the first is dark-first and already answers the no-attribute case, the second overrides no tokens.

### Docs

- **Changed** `site/content/styling.md` documents that `applyTheme` publishes the PREFERENCE (so `'system'` removes the attribute) and writes no `.dark` class, and its dark-override section now describes the landed state: `[data-theme='dark']` is the spelling this package writes, `.dark` is supported for consumers who set it themselves (#242), the media guard no longer outranks either (#241), and a twin block under that guard is still required for the `'system'` preference because no selector a consumer can write matches in that cell. The top-of-page claim that a shadcn theme 'pastes in verbatim' is corrected: the `:root` half does, the dark half needs one find-and-replace.
...
> **Note:** `@llui/components@0.11.4` and `@llui/components@0.11.5` were incomplete publishes — the new `utils/roving` module was missing from the packed `dist/`. Root cause: `pnpm publish` runs no build (the packages carried no `prepack`/`prepare` hook), so it packed a stale `dist/` left on disk from an earlier checkout. `0.11.6` is the corrected publish, and every publishable package now carries a `prepack: "pnpm run build"` hook so `pnpm publish`/`pnpm pack` always build fresh from source (and fail loud rather than ship stale). Use `0.11.6`; do not use `0.11.4` or `0.11.5`.

### `@llui/components@0.11.6`
