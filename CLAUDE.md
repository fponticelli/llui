...
Twenty-two packages under `packages/`, managed by pnpm workspaces + Turborepo:
...
- **v2 Compiler Architecture** (`docs/proposals/v2-compiler/`) — partially realized by the signals migration: `@llui/compiler` is a standalone package (extracted from `@llui/vite-plugin`), and cross-file Msg/State/Effect analysis is live (`cross-file-resolver.ts`, wired into the vite-plugin). The **live** transform path is `transformSignalComponentSource` (a single string-edit signal transform); it emits agent/devtools metadata **inline** (its own `sharedMetaProps`), NOT through a registry.
  ...
  ⚠️ **Dormant, present-but-not-wired code — do not assume these are on the shipping path:**
  - The compiler-module registry (`module.ts` `CompilerModule`/`ModuleRegistry`) and the introspection/devtools factories (`introspection-factory.ts`) are **registered but never invoked** — `getIntrospectionFactory`/`getDevtoolsFactory` have zero `src` call sites and `ModuleRegistry` is never instantiated in the active pipeline.
  - The cross-package library ABI (`__llui_deps.json`, schema v2): the producer (`build-manifest.ts` + `scripts/emit-deps.mjs`) runs **only out-of-band** (not in any `turbo build` / package build script; some `dist/__llui_deps.json` artifacts exist from manual runs). The consumer resolution (`manifest-resolve.ts`) lives only in the **dormant** cross-file walker (`walkProgram`/`cross-file-walker.ts`, reachable only from `packages/compiler/scripts/walker-validate.mjs` + tests). So cross-package narrowing of `state.map(s => libHelper(s))` through a precompiled package is **not active** in the live Vite transform, despite `docs/publishing-a-precompiled-library.md` describing it as shipped.
  - The `track({ deps })` compiler escape hatch (`track-utils.ts`) and the `collect-deps.ts` / FULL_MASK mask model belong to that same dormant walker path, not the live transform.
