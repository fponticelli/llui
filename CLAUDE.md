...
Twenty-two packages under `packages/`, managed by pnpm workspaces + Turborepo (21 published `@llui/*`/`llui-agent`; `@llui/agent-e2e` is private test fixtures):
...
| `@llui/vite-plugin`                    | Vite adapter: wires compiler into Vite (diagnostics via this.error()). Has grown a full dev-server surface too — the notes HTTP router/middleware/store, the attention router, agent dev endpoints, HUD injection (a decomposition into composed named plugins is tracked as tech-debt).     | peer: vite                                                                                                     |
...
| `@llui/devmode-annotate`               | Dev-mode HUD: capture-only overlay that drops annotated notes from the running app into the shared notebook for the LLM                                                                                                                                                                      | @llui/dom (peer), @llui/notes-format                                                                           |
...
- **The `draining` reentrancy guard must wrap EVERY commit path.** A commit can re-enter `send` (a `blur` fired when the commit removes a focused node). Every path that calls `commitPending()` — the send drain, `flushFrame`, devtools `setState`, and the post-mount replay — must set `draining` first, or the reentrant send starts a NESTED drain mid-reconcile and corrupts the scope tree / `removeBetween` into a `NotFoundError`. `drain()` must also bail once `disposed`.
