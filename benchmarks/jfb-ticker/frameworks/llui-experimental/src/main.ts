...
const App = component<State, Msg, never>({
  ...
  view: ({ send, text, each }) => [
    div({ id: 'ticker' }, [
      ...
      // ── Symbol list (EXPERIMENTAL — fully-static cells) ──
      // Identical DOM shape (200 keyed <tr> rows, 6 <td> cells each) and
      // same `mode-*` class binding on the row — but each <td>'s text is
      // a raw DOM text node built at row construction with the symbol's
      // initial value, NOT a `text(item(...))` reactive binding. This
      // eliminates the per-row text bindings from the flat binding array
      // entirely (~1200 bindings removed), so Phase 2's scan only walks
      // the 32 dashboard bindings + 200 row-class bindings ≈ 232. The
      // per-case-binding-index optimization would deliver the same effect
      // for the narrow case automatically.
      table({ id: 'symbols', class: 'ticker' }, [
        tbody(
          each<SymbolRow>({
            ...
            render: ({ item }) => {
              const sy = item.current()
              return [
                tr({ class: (s: State) => `mode-${s.dashboard.displayMode}` }, [
                  td([document.createTextNode(sy.ticker)]),
                  td({ class: 'col-price' }, [document.createTextNode(sy.price.toFixed(2))]),
                  td({ class: 'col-change' }, [document.createTextNode(sy.change.toFixed(2))]),
                  td({ class: 'col-change' }, [document.createTextNode(sy.changePct.toFixed(2))]),
                  td({ class: 'col-volume' }, [document.createTextNode(String(sy.volume))]),
                  td([document.createTextNode(String(sy.lastTickAt))]),
                ]),
              ]
            },
          }),
