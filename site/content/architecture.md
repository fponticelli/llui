...
3. **Output equality.** A _value_ binding that passes the gate runs its accessor; the commit fires only if the value actually changed (`Object.is` against the last value). A coarse dependency wastes an accessor call but never a DOM write.

Structural primitives — `show`, `branch`, `each` — are not plain bindings. Each registers a structural binding gated on its own deps, but its commit _reconciles_ (swaps an arm, diffs keyed rows) and owns child scopes. Both kinds live in the same sweep; structural reconciles and binding commits happen as their deps dirty. Step 3 does **not** apply to them: a structural binding's accessor is the identity function (its commit needs the whole state to mount an arm or a row against), so an equality check would compare the state object's identity rather than the binding's output — and inside an `each` row that object is one of two recycled per-row buffers. A structural binding therefore reconciles whenever its gate passes, and de-duplicates internally instead (an unchanged arm key is a no-op; `each` diffs its keyed rows).

`foreign()` creates an opaque container for third-party imperative components (ProseMirror, Monaco, etc.) — LLui owns the container but not its contents; declared `state` signals materialize to `LiveSignal`s that the imperative `mount` callback binds.
...
There is **no fixed path budget.** A 200-path component uses 7 chunks, and each binding's gate is still a handful of integer ANDs over the chunks its mask touches — ~constant regardless of total path count. (This replaces the older fixed two-word `mask`/`maskHi` design and its 62-path limit, which was deleted along with the legacy runtime.) Update cost scales with what changed, not with state depth, so a large flat state is fine — reach for view functions (sliced signals) first. When a widget's state is nobody else's business, give it its own update loop with `island({ def })` — see the three state tiers in [Composition Patterns](/composition-patterns), which carry the measured cost (~2.4x at mount, ~2x cheaper per host update, because an island is not a child scope the host reconciler walks).
...
The runtime helpers a lowered view calls are pulled in by an `import { … } from '@llui/dom'` the compiler prepends. Those are top-level bindings, so if your file already uses a helper's name anywhere — `const el = document.createElement('span')`, `function el() {}`, an `import { el } from './my-helpers.js'`, even a `const el` declared inside the view itself — the compiler imports that helper under an alias (`import { el as el$llui }`) and emits the alias at the lowered call sites. Your own `el` keeps meaning yours everywhere, including inside the view.

### Compile-time correctness rules

The framework's correctness, agent-protocol, and convention rules are non-bypassable compile-time **errors** surfaced by the Vite plugin (LLMs ignore warnings, so a build that fails closed is the only effective channel). The signal-specific rules catch the reactivity foot-guns: `peek-in-slot` (a `.peek()` snapshot used as a reactive slot value), `operator-on-signal` (`signal + 1`, ternary on a signal), `no-node-construction-in-body` (element/text helper called inside a `.map` body), `pure-derive-body` (side effects or reactive primitives inside a `.map` body), `prefer-at-over-map` (a plain single-field projection `sig.map(p => p.x)` that should be `sig.at('x')`), `at-after-map` (`sig.map(fn).at('x')`, which has no static path to slice), `empty-props` (`div({}, […])` — the element helpers already take the children-only call `div([…])`, so the empty literal is a throwaway allocation on the element mount path; the diagnostic carries the rewrite as a fix), and `agent-annotation-syntax` (a malformed agent annotation — `@routeGated("state.mode === "admin"")` and friends: the argument grammar drops what it cannot read unambiguously, and a dropped `@routeGated` is an ungated action while a dropped `@validates` is an unchecked field, both invisible at runtime), and `tag-send-drift` (a hand-written `tagSend(send, ['touch'], () => send({type: 'touched'}))` whose variant list disagrees with the `type` its own handler dispatches — that list becomes `__lluiVariants`, which the agent/devtools surface reads to tell an LLM which Msg variants a control can emit, so a drifted tag lies to the agent with no runtime symptom), and `imperative-dom-mutation` (a write to `textContent` / `innerHTML` / `className` / `classList` / `style` / `dataset` / `setAttribute` on a node reached from an event handler's own parameter, inside an element helper's `on*` prop — `view()` runs once and builds bindings, so a node "no binding depends on" is still owned by the scope that built it and can be moved, reused or torn down by `show`/`branch`/`each`, at which point the write is silently lost or lands on the wrong node; bind the value reactively, or mount the widget as an island, and put a genuinely imperative surface in `foreign()`, which the rule exempts along with `island()`/`subApp()`. The handler is read whether it is written inline, wrapped in `tagSend`, or referenced by the name of a module-scope `const`/`function`; a path that leaves the document subtree the view was built into — through `ownerDocument`, `e.view`, `getRootNode()` and friends — is not the view's node and is not reported, so a theme toggle or a scroll lock reached that way stays legal). (There is deliberately **no** whole-`state`-coarseness rule: rendering a whole-state object is already a type error, and a `Signal` coerced in an operator is caught by `operator-on-signal`.)

**What counts as a signal is decided by import provenance, not by a name.** `peek-in-slot` and `operator-on-signal` both ask "is this expression a signal?", and the answer has to cover the two calls that produce one out of thin air — `derived(inputs, fn)` and `constant(value)`. Which `derived`/`constant` a call site means is a question about the file's imports, so it is resolved through the same `HelperBindings` gate that decides whether `div(…)` is the framework's `div`: a canonical, aliased or namespace `@llui/dom` import counts, and a module-scope declaration of your own, a lexical shadow, an import from anywhere else, and anything unresolvable do **not**. It fails closed — when provenance cannot be established you get silence, never a diagnostic — because a missed lint costs one foot-gun while a false positive breaks a build for code that did nothing wrong. Asking the identifier's spelling instead was wrong in both directions at once: it reported a consumer's own `derived()`, and it did not know `constant()` at all, so `text(constant('') || 'FALLBACK')` compiled clean and rendered `""` with the fallback silently swallowed.
