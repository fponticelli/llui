...
```typescript
export declare function firstEnabled(
...
```typescript
export declare function focusRovingItem(
  origin: Element | null,
  scope: string,
  value: string,
  opts?: {
...
```typescript
export declare function getNestedLayers(
...
```typescript
export declare function isInNestedLayer(
  target: Node | null,
  aspect?: NestedLayerAspect,
  within?: NestedLayerScope,
): boolean
...
```typescript
export declare function lastEnabled(
  items: readonly string[],
  disabled: readonly string[],
): string | null
```
...
```typescript
export declare function nextEnabled(
  items: readonly string[],
  disabled: readonly string[],
  from: string,
  delta: 1 | -1,
...
```typescript
function presenceEndHandler<E extends AnimationEvent | TransitionEvent>(
  handler: (e: E) => void,
): (e: E) => void
```
...
```typescript
export declare function registerNestedLayer(
  source: ElementSource,
  opts?: NestedLayerOptions,
): () => void
```
...
```typescript
export declare function resolveRovingMove(
  key: string,
  current: string,
...
```typescript
export declare function resolveTextDirection(
...
```typescript
function typeaheadMatchByItems(
  items: string[],
  disabled: readonly string[],
  query: string,
  startFrom: number | null,
...
```typescript
function validateSchemaAsync<T>(
...
view: ({ state, send }) => {
  ...
  return [
    div({ ...parts.root, class: inViewState.at('visible').map((v) => (v ? 'fade-in' : '')) }, [
      onMount((el) => inView.createObserver(el, inViewSend, { threshold: 0.5, once: true })),
    ]),
  ]
}
```
...
- Single-character queries advance past the current position (jump to
  the _next_ item starting with that letter), which is the standard
...
```typescript
function presenceEndProps<M extends { type: string }>(
  send: (msg: M) => void,
  animationMsg: M,
...
```typescript
function searchableSelectUpdate(
