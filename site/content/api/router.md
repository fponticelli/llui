...
Query parameters are explicit. `repeatedRouteCodec()` represents ordered repeated query values and custom rest parameters. Scalar query codecs reject duplicate values. Unknown query keys are ignored when matching and omitted by generation. Defaults are present after matching, optional while generating, and omitted from the canonical URL; that rule also applies to rest defaults. Object and array defaults are cloned into each returned location, so mutating one result cannot affect another.
