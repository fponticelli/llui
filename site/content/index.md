...
| [`@llui/router`](/api/router)                                   | Routing — named type-safe routes, Standard Schema codecs, history/hash mode, guards, link helper                                                                               |
| [`@llui/transitions`](/api/transitions)                         | Animation helpers — `transition()`, `fade`, `slide`, `scale`, `collapse`, `flip`, `spring`                                                                                     |
| [`@llui/components`](/api/components)                           | 66 headless components + shadcn-compatible CSS tokens and an opt-in baseline stylesheet                                                                                        |
| [`@llui/cli`](/api/cli)                                         | `llui add` — copies shadcn/ui-styled component source from the registry into your project                                                                                      |
| [`@llui/interactions`](/api/interactions)                       | Standalone focus, dismissal, floating-positioning, modal-isolation, scroll-lock, direction, and roving-focus primitives                                                        |
| [`@llui/test`](/api/test)                                       | Test harness — testComponent, testView, propertyTest, replayTrace                                                                                                              |
| [`@llui/security`](/api/security)                               | Shared URL + loopback-origin sanitization for the DOM-sink and dev-server security surfaces                                                                                    |
...
| [`@llui/devmode-annotate-editor`](/api/devmode-annotate-editor) | Optional rich Markdown editor upgrade for the annotation HUD; auto-registered by the Vite plugin when installed                                                                |
