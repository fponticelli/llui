...
| `llui_lint`               | Lint TypeScript source against `@llui/eslint-plugin`'s 21 rules — pass a `path` to a file. Returns violations + score. Lets an LLM self-correct without running a build. |
...
`@llui/agent` ships a set of Level 1 slices that let a host app embed an in-app conversational surface for the user's external LLM (Claude Desktop / IDE / wherever the MCP bridge is mounted). The protocol itself is bring-your-own-LLM — one LLM per user, cross-app context, no per-app cost — but the framework provides the _surface_ so the user doesn't alt-tab to talk to their agent.

Five slices compose into one panel:
...
| `agentLog`       | Ring-buffered activity timeline (every rpc, with origin tag + payload detail + diff) |
| `agentAttention` | Current dispatch's spotlight: which DOM regions to flash when state changes          |
| `agentChat`      | Input composer state (pending text, submit-in-flight, Enter handling)                |

Each is a `init` / `update` / `Msg` / `connect()` triple. The host slots them into its app state via `sliceHandler`, routes inbound `Append { entry }` Msgs from the WS log-append channel to BOTH `agentLog` and `agentAttention` (the framework fans out automatically through the factory), and spreads the prop bags into its own layout.
...
//   { type: 'agent', sub: 'chat',    msg: agentChat.AgentChatMsg }
...
createAgentClient<State, Msg>({
  ...
  slices: {
    ...
    wrapChatMsg: (m) => ({ type: 'agent', sub: 'chat', msg: m }),
  },
})
```
...
import { div, input, button, text, each } from '@llui/dom'
import { agentChat, summarizeDiff } from '@llui/agent/client'

function panel(send: Send<Msg>) {
  const chatBag = agentChat.connect<State>(
    (s) => s.agent.chat,
    (m) => send({ type: 'agent', sub: 'chat', msg: m }),
  )
  return div([
    ...
    // Chat composer — the bag carries Enter handling, double-submit guard, etc.
    div([
      input({ ...chatBag.input, placeholder: 'Talk to Claude…' }),
      button({ ...chatBag.submitButton }, [text('Send')]),
    ]),
  ])
}
```
...
| Long-poll for the user's typed reply     | `wait_for_user_input`      |
| Push prose into the activity feed        | `narrate`                  |
| Wait for a specific state path to change | `wait_for_change`          |

The conversational loop typically reads as: `narrate("about to do X") → send_message(...) → narrate("here's the result") → wait_for_user_input()`. The user reads the narration inline with the dispatches; the user's reply lands as a `user-input` log entry next to the narrations; the agent picks up the reply and continues.
