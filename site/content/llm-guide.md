...
- **MCP debug tools:** In dev mode, `window.__lluiDebug` exposes the full debug API — core inspection (`getState`, `send`, `getBindings`, `whyDidUpdate`, `decodeMask`, `getMessageSchema`, `snapshotState`/`restoreState`, `exportTrace`), DOM inspection (`inspectElement`, `getRenderedHtml`, `getFocus`, `dispatchDomEvent`), binding/scope tools (`forceRerender`, `getEachDiff`, `getScopeTree`, `getDisposerLog`, `getBindingGraph`), effect control (`getPendingEffects`, `getEffectTimeline`, `mockEffect`, `resolveEffect`), time travel (`stepBack`), coverage (`getCoverage`), and page eval (`evalInPage`). The `@llui/mcp` package wraps each as an MCP tool — run `npx @llui/mcp` alongside the dev server and connect via any MCP client (Claude Desktop, Claude Code) for interactive debugging. The relay connects on page load if the MCP server is running; otherwise call `__lluiConnect()` from the console.
